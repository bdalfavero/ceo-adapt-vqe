...
def _(df, df_adapt, plt):
    ...
    right_ax2.plot(df_adapt.index, df_adapt["no_barrier_depths"], color="red", linestyle="-.", label="No barriers, transpiled")
    right_ax2.legend()
    ax[2].set_yscale("log")
...
def _(df, df_adapt, plt):
    ...
    ax2.scatter(df_adapt["no_barrier_depths"], df_adapt["error"], color="tab:red", marker="o", label="ADAPT (no barriers)")
    ax2.legend()
    ax2.set_yscale("log")
    # ax2.set_xscale("log")
